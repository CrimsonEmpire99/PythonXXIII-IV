            ...
            if condition_value and not executed_block:
                ...
                while j < len(block):
                    ...
                    if subline.strip().startswith('if '):
                        # Find the actual index in the original lines list
                        actual_index = i + 1 + j
                        skip_to = conditional_statement(lines, actual_index, variables, python_keywords)
                        # Calculate how many lines to skip in the block
                        j = skip_to - (i + 1)
                    else:
...
while True:
    ...
    if inp.endswith('/run'):
        ...
        # Process all lines, handling conditionals
        i = 0
        while i < len(cmd_lines):
            ...
            if line.strip() and not line.endswith('/run'):
                ...
                if line.strip().startswith('if '):
                    i = conditional_statement(cmd_lines, i, variables, python_keywords)
                else:
